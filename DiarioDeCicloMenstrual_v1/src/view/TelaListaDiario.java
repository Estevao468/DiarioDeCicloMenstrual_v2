...
public class TelaListaDiario implements ActionListener, ListSelectionListener {
    ...
    /**
     * Método para listagem de todos dados cadastrados no sistema
     * @param d dados referentes a classe ControleDados
     * @param op opção de dados escolhida pelo usuário
     * 
     */
	public void mostrarDados(ControleDados d, int op){
		...
		switch (op) {
		case 1: //mostrar lista de ciclos cadastrados
            listaNomeCiclo = new ControleCiclo(dados).getNomeCiclo(); // chamando nomes de ciclos cadastrados da controle
            listaCicloCadastrados = new JList<String>(listaNomeCiclo);// criando a lista de nomes de ciclos cadastrados
		...
		case 2://mostrar lista de sintomas cadastrados
            listaNomeSintoma = new ControleSintoma(dados).getNomeSintoma(); // chamando nomes de sintomas cadastrados da controle
            listaSintomaCadastrados = new JList<String>(listaNomeSintoma); // criando a lista de nomes de sintomas cadastrados
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        if(e.getValueIsAdjusting() && src == listaCicloCadastrados){ // seleciona ciclo
            new TelaDetalheCiclo().inserirEditar(2, dados, listaCicloCadastrados.getSelectedIndex());
            //janela.dispose();
        }

        if (e.getValueIsAdjusting() && src == listaSintomaCadastrados) { // seleciona sintoma
